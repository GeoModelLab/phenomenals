...
phenologyCalibration <- function(weather_data,
  ...
  # Check referenceBBCH
  if (!is.data.frame(referenceBBCH)) stop("❌ 'referenceBBCH' must be a data frame.")
  if (!"Site" %in% names(referenceBBCH)) stop("❌ 'referenceBBCH' must contain a 'Site' column.")
  if (!"Variety" %in% names(referenceBBCH)) stop("❌ 'referenceBBCH' must contain a 'Variety' column.")
  ...
  pkg_path <- system.file("", package = "phenomenals")
   ...
   # dir.create(work_dir, recursive = TRUE)
    ...
    if (identical(sites, "all")) {
      ...
      if (length(sites) == 0) stop("❌ No overlapping sites between weather_data and referenceBBCH")
    }
    ...
    if (nrow(missing_combos) > 0) {
      stop(
        paste0(
          "❌ Missing experiments in referenceBBCH for the following site-variety combinations:\n",
    ...
    for (s in site_names) {
      ...
      if (length(date_col) == 0) stop("❌ 'weather_data' must contain a 'DateTime' or 'Date' column.")
      ...
      if (anyNA(weather_data$date)) {
        stop("❌ Some 'date' entries in weather_data could not be parsed with format '%m/%d/%Y'. Check for typos or inconsistencies.")
      }
      ...
      message(glue::glue("🌦️  Weather data written for site '{s}' ➜ {basename(weather_file)}"))
    }
    ...
    output <- tryCatch({
      ...
      stop("❌ C# model execution failed: ", e$message)
    })
    ...
    if (any(grepl("no experiment available", tolower(output)))) {
      stop("❌ Calibration failed: No experiment available for at least one variety-site combination.")
    }
