...
#'
#' This function runs the Phenomenals method by passing weather data and phenological observations (BBCH) directly from R to the underlying C# executable.
...
#' @param target_data A data frame containing the target data (e.g., yield, brix, berry number, malic acid). The dataframe must include: `Variety`, `Site`, `Latitude`, `Longitude`, `Date`, and `BBCH`.
...
#'
#' @return A named list of data frames containing model outputs, including predictions, diagnostics, coefficients, and processed phenological signals.
#'
#' @examples
#' \dontrun{
...
runPhenomenals <- function(weather_data,
  ...
  #TODO: CHANGE
  exe_path<-'C:\\Program Files\\R\\R-4.4.2\\library\\phenomenals\\bin'
  list_files_out <- list.files(
    path = file.path(dirname(exe_path),
                     "outputsValidationDaily"),
    pattern = "\\.csv$",
    full.names = TRUE
  )
  ...
  if (!"Site" %in% names(weather_data)) stop("❌ 'weather_data' must contain a 'Site' column.")
  ...
  # CONFIGURATION----
    ...
    df <- df_sim |>
      dplyr::mutate(
        ...
        year_meteo = dplyr::case_when(
          hemisphere == "southern" & DOY >= 60 ~ lubridate::year(Date) - 1,  # Mar–Dec → year_0 = previous year
          hemisphere == "southern" & DOY < 60  ~ lubridate::year(Date),      # Jan–Feb → year_0 = current year
          hemisphere == "northern"             ~ lubridate::year(Date),
          TRUE ~ NA_integer_
        )
      )
    ...
    yieldData <- yieldData_raw |>
      ...
      dplyr::mutate(
        target_0 = 2 * (value - min(value, na.rm = TRUE)) /
    ...
    harvest_map_extended <- yieldData |>
      dplyr::mutate(harvest_date = dplyr::case_when(
      ...
      dplyr::mutate(
        year_0 = harvest_year,
        year_1 = harvest_year - 1
      ) |>
      tidyr::pivot_longer(cols = starts_with("year_"),
    ...
    filter_by_completion_ranges_dt <- function(df, ranges) {
      ...
      keep_rows <- Reduce(`|`, lapply(ranges, function(rng) {
        dt[, Completion >= rng[1] & Completion <= rng[2]]
      }))
    ...
    df_cumulative_signal_all <- weighted_signals_all  |>
      filter_by_completion_ranges_dt(evaluation_range) |>
      dplyr::mutate(cyclePerc = bin +
                      ifelse(relative_year == "year_1", 0, 200))  |>
    ...
    for (i in seq_len(nrow(x_pos_list))) {
      ...
      if (nrow(df_x) < 10) next

      full_model <- lm(target_0 ~ ., data = df_x)
    ...
    model_results_variety_loocv <- signal_index_wide_all_with_targets |>
      ...
      purrr::map(function(df_group) {
        ...
        all_years <- unique(df_group$harvest_year)
        year_count <- length(all_years)

        valid_cyclePercs <- df_group |>
          dplyr::group_by(cyclePerc) |>
          dplyr::summarise(n_years = dplyr::n_distinct(harvest_year), .groups = "drop") |>
          dplyr::filter(n_years >= multicollinearity_threshold * year_count) |>
        ...
        results_per_x <- df_group |>
          ...
          purrr::map(function(df_variety) {
            ...
            if (nrow(df_variety) < 4) {
              cat(glue::glue("\r⚠️  Skipped (n = {nrow(df_variety)}): {this_variety} | x = {this_x}              \n"))
              flush.console()
              return(NULL)
            }
            ...
            tryCatch({
              ...
              if (is.null(predictors) || length(predictors) == 0) {
                cat(glue::glue("\r⚠️  No predictors found for {key} — skipping.\n"))
                return(NULL)
              }
              ...
              pred <- suppressWarnings(predict(model, newdata = df_clean))
              df_clean$pred <- pred

              r2 <- cor(df_clean$target_0, pred, use = "complete.obs")^2
              rmse_val <- sqrt(mean((df_clean$target_0 - pred)^2, na.rm = TRUE))
              mae_val  <- mean(abs(df_clean$target_0 - pred), na.rm = TRUE)

              # RelAIMPO first to define `relimpo_failed`
              relimpo_result <- tryCatch({
                rel_model <- lm(model_formula, data = df_clean)
                rel <- relaimpo::calc.relimp(rel_model, type = "lmg", rela = TRUE)
              ...
              # Now print message
              msg <- glue::glue(
                "🔄 LOOCV for {this_variety} | {this_site} | Cycle% = {this_x} | target = {this_target} | R² = {round(r2, 2)} | RMSE = {round(rmse_val, 2)}"
              )

              if (relimpo_failed) {
                msg <- paste0(msg, " ⚠️ (relimpo skipped)")
              }

              # Pad with spaces to overwrite leftover text from previous lines
              cat("\r", msg, strrep(" ", 20), sep = "")
              flush.console()

              coef_df <- broom::tidy(model$finalModel) |>
                dplyr::mutate(variety = this_variety, site = this_site, cyclePerc = this_x, target = this_target)
              ...
              message(glue::glue("❌ Model error for {this_variety} | {this_site} | Cycle% = {this_x}: {e$message}"))
              return(NULL)
            })
          }) |>
          purrr::compact()
        ...
        diagnostics_cv <- diagnostics_tbl |>
          ...
          dplyr::group_by(site, variety, target) |>
          dplyr::mutate(
            R2_roll_mean = dplyr::cummean(R2),
            R2_roll_sd   = purrr::map_dbl(seq_along(R2), ~ if (.x == 1) NA_real_ else stats::sd(R2[1:.x])),
            R2_roll_cv   = R2_roll_sd / R2_roll_mean) |>
          dplyr::ungroup()

        results_per_x <- purrr::map(results_per_x, function(res) {
          this_diag <- dplyr::filter(
            diagnostics_cv,
